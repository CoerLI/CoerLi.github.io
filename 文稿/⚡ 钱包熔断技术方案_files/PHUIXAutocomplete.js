...
display=JX.$N('span',{},[icon,map.displayName]);JX.DOM.alterClass(display,'tokenizer-result-closed',!!map.closed);map.display=display;return map;},_deactivate:function(){var
...
pattern=new
RegExp(prefix);if(!trim.match(pattern)){return;}trim=trim.replace(pattern,'');trim=trim.trim();}this._value=trim;var
